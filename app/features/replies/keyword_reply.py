...
# --- Regex เช็กว่าเป็นข้อความที่ดูเหมือนสัญลักษณ์เหรียญ (เช่น BTC, ETH, sol/usdt) ---
_COIN_LIKE = re.compile(r"^[A-Za-z0-9:/\- ]{2,20}$")

def get_reply(text: str) -> Optional[str]:
    """Mapping ข้อความ → คำตอบ (รองรับทั้งแบบ exact และทักทาย 'สวัสดี')
    หมายเหตุ: ถ้าดูเหมือนสัญลักษณ์เหรียญ จะคืน None เพื่อให้ไปเข้าชั้นราคาหรือวิเคราะห์แทน
    """
    if not text:
        return None
    raw = text.strip()
    ...
    # 0) ถ้าข้อความดูเหมือนสัญลักษณ์เหรียญ → ข้าม keyword layer
    if _COIN_LIKE.fullmatch(raw):
        return None

    # 1) จับ 'สวัสดี'
    if _GREET_RE.search(raw):
        return "สวัสดีครับ : )"

    # 2) exact-match กับ KEYWORD_MAP
