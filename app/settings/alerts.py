...
class AlertSettings(BaseSettings):
    ...
    symbol: str = Field(default="BTC", env="ALERT_SYMBOL")                # เหรียญ default
    threshold_pct: float = Field(default=5.0, env="ALERT_THRESHOLD_PCT")  # % เปลี่ยนแปลงที่จะทริกเกอร์
    poll_sec: int = Field(default=60, env="ALERT_POLL_SEC")               # รอบตรวจสอบ (วินาที)
    cooldown_sec: int = Field(default=1800, env="ALERT_COOLDOWN_SEC")     # คูลดาวน์ (วินาที)
    hysteresis_pct: float = Field(default=1.0, env="ALERT_HYSTERESIS_PCT")# กันสัญญาณสั่น
