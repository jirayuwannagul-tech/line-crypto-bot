...
# app/analysis/entry_exit.py
# =============================================================================
# ENTRY/EXIT LOGIC + TEXT FORMAT
# =============================================================================

from typing import Dict, Any

def suggest_trade(df, symbol: str, tf: str, cfg: Dict[str, Any]) -> Dict[str, Any]:
    """
    วิเคราะห์ trade และคืน dict
    """
    last_close = float(df["close"].iloc[-1])

    # --- คำนวณ SL และ TP ---
    sl = round(last_close * (1 - 0.03), 2)   # Stop Loss –3%
    tp1 = round(last_close * (1 + 0.03), 2)  # TP1 +3%
    tp2 = round(last_close * (1 + 0.07), 2)  # TP2 +7%
    tp3 = round(last_close * (1 + 0.12), 2)  # TP3 +12%

    # --- วิเคราะห์ indicator คร่าว ๆ ---
    rsi = float(df["rsi"].iloc[-1]) if "rsi" in df else None
    ema20 = float(df["ema20"].iloc[-1]) if "ema20" in df else None
    ema50 = float(df["ema50"].iloc[-1]) if "ema50" in df else None

    reason = []
    if rsi:
        if rsi > 70:
            reason.append("RSI สูง → อาจ overbought")
        elif rsi < 30:
            reason.append("RSI ต่ำ → อาจ oversold")
        else:
            reason.append(f"RSI {rsi:.1f} → ปกติ")

    if ema20 and ema50:
        if ema20 > ema50:
            reason.append("EMA20 > EMA50 → แนวโน้มขาขึ้น")
        else:
            reason.append("EMA20 < EMA50 → แนวโน้มขาลง")
...
def format_trade_text(trade: Dict[str, Any]) -> str:
    """
    สร้างข้อความสรุปสำหรับส่ง LINE
    """
    ...
    return (
        f"📊 {trade.get('symbol')} {trade.get('tf')} — สรุปสัญญาณ\n\n"
        f"🎯 Entry: {entry}\n"
        f"• SL: {sl} (–3%)\n"
        f"• TP1: {tp1} (+3%)\n"
        f"• TP2: {tp2} (+7%)\n"
