...
async def tick(
    ...
    debug: bool = Query(False, description="True = คืน payload เต็ม"),
):
    ...
    if debug:
        # คืนผล payload + message เต็ม สำหรับ debug/manual
        return {"ok": True, "ran": True, "dry_run": dry_run, "results": result}

    # ปกติ: คืน status สั้น ๆ (สำหรับ Cloud Scheduler)
